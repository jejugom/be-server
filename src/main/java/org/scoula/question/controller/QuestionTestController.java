...
public class QuestionTestController {
    ...
    public ResponseEntity<QuestionResponseDto> handleTextQuestion(
        ...
        String gptPrompt = buildGptPrompt(text.trim());
        log.debug("넣을 텍스트: {}", gptPrompt);
        String aiResponse = callGptApi(gptPrompt);
        QuestionResponseDto response = new QuestionResponseDto("SUCCESS", "텍스트 질문이 성공적으로 처리되었습니다.", text.trim(), aiResponse);
    ...
    public ResponseEntity<QuestionResponseDto> handleVoiceQuestion(
        ...
        try {
            ...
            // ******* GPT 서비스 로직 ********
            // 아래 내용은 예시 작성이니 아예 다 바꾸셔도 됩니다!

            // 2. 변환된 텍스트로 GPT 프롬프트 준비
            String gptPrompt = buildGptPrompt(speechToText);
            log.debug("GPT에 넣을 텍스트: {}", gptPrompt);
            ...
            QuestionResponseDto response = new QuestionResponseDto("SUCCESS", "음성 질문이 성공적으로 처리되었습니다.", speechToText, aiResponse);
            return ResponseEntity.ok(response);

        } catch (Exception e) {
            log.error("음성 질문 처리 실패: {}", e.getMessage(), e);
            QuestionResponseDto errorResponse = new QuestionResponseDto("ERROR", "음성 질문 처리 중 오류가 발생했습니다: " + e.getMessage());
