...
public class KakaoAuthService {
  ...
  private static final String LOG_PATH = "/Users/wina/Documents/github/nohudorak/be-server/logs/kakao_log.csv";
  ...
  public KakaoLoginResponseDto processKakaoLogin(String code) {
    ...
    try {
      ...
      resultCode = "ERROR_" + e.getClass().getSimpleName();
      log.error("❌ 카카오 로그인 실패: {}", e.getMessage(), e);
      throw new RuntimeException("카카오 로그인 처리 실패", e);

    } finally {
      long end = System.currentTimeMillis();
      log.info("⏱️ 카카오 로그인 처리 완료, 소요 시간: {}ms, 결과 코드: {}", end - start, resultCode);
      logKakaoExecution("processKakaoLogin", end - start, resultCode);
    }
  }
   ...
   * 신규 회원 여부 판단
   * <p>
   * 사용자가 신규 회원인지 기존 회원인지 판단합니다. 첫 개인 설정 정보가 입력되지 않은 경우 신규 회원으로 간주하는 로직 추가가 필요합니다.
   *
  ...
  private void logKakaoExecution(String phase, long duration, String resultCode) {
