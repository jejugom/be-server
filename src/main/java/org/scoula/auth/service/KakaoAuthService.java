...
public class KakaoAuthService {
	...
	public KakaoLoginResponseDto processKakaoLogin(String code) {
		...
		if (!lastProcessedUserIsNew) {
			Double tendency = user.getTendency();
			lastProcessedUserTendencyNotDefined = (tendency == null || tendency == 0.0);
		}
	...
	/**
	 * [신규 추가] 이메일로 사용자의 표시 이름을 조회합니다.
	 * AuthController의 getProfile 메소드에서 사용됩니다.
	 *
	 * @param email 조회할 사용자의 이메일
	 * @return 사용자의 표시 이름 (userName)
	 * @throws UsernameNotFoundException 해당 이메일의 사용자를 찾을 수 없을 때
	 */
	@Transactional(readOnly = true)
	public String getDisplayNameByEmail(String email) {
		// UserRepository를 사용하여 이메일로 User 엔티티를 찾습니다.
		UserVo user = Optional.ofNullable(userMapper.findByEmail(email))
			.orElseThrow(() -> new UsernameNotFoundException("사용자를 찾을 수 없습니다: " + email));

		// 찾은 User 엔티티에서 userName을 반환합니다.
	 ...
	 * <p>
	 * 인가 코드로 카카오 사용자 정보를 조회하여 신규 회원 여부를 사전에 확인합니다. DB 저장 전에 신규 회원 여부를 판단하여 올바른 결과를 반환합니다.
	 *
	 * @param code 카카오에서 발급한 인가 코드
